...
class HabitRepositoryTest {
    ...
    fun `mapper from db working correct`() = runBlocking<Unit> {
        ...
        //act
        var testResult = HabitResult.ValidResult(emptyList(), emptyList())
        repository.getHabitsCache().collect {
            testResult = it as HabitResult.ValidResult
        }

        assertEquals(habitList, testResult.bad?.plus(testResult.good)?.get(0))
